...
- Error responses should include a human-readable `detail` string, and where supported by the framework, an explicit integer `status_code`. For FastAPI endpoints using plain `HTTPException` without a global exception normalizer, the default `{ "detail": string }` shape is acceptable.
