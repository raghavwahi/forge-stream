...
from app.schemas.github_repos import ListReposResponse
...
def _get_repos_service(
    ...
    return GitHubReposService(app_service)
...
async def list_repositories(
    ...
    try:
        repos = await service.list_repos_for_installation(installation_id)
    except Exception as exc:
        logger.error("Failed to list repositories: %s", type(exc).__name__)
        raise HTTPException(
...
async def search_repositories(
    ...
    try:
        ...
        logger.error("Failed to search repositories: %s", type(exc).__name__)
        raise HTTPException(
            status_code=status.HTTP_502_BAD_GATEWAY,
            detail="Failed to fetch repositories from GitHub",
        ) from exc
